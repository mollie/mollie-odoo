...
class PaymentIcon(models.Model):
    ...
    def onchange_provider_ref(self):
        if (
            self.provider == "mollie"
            and self.acquirer_reference
            and DEFAULT_METHOD_VALUES.get(self.acquirer_reference, False)
        ):
    ...
    def get_available_methods(self, amount, currency, partner_id):
        ...
        available_list = []
        for icon in self:
            if amount > icon.maximum_amount or amount < icon.minimum_amount:
                continue
            if not icon.currency_ids and not icon.country_ids:
                ...
                if (
                    currency in icon.currency_ids
                    and country in icon.country_ids
                ):
                    available_list.append(icon)
