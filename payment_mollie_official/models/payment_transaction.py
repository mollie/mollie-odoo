...
class PaymentTransaction(models.Model):
    ...
    def create(self, values_list):
        ...
        for transaction in transactions:
            if transaction.provider == 'mollie' and transaction.mollie_payment_method:
                mollie_method = transaction.acquirer_id.mollie_methods_ids.filtered(lambda method: method.method_code == transaction.mollie_payment_method)
    ...
    def _process_feedback_data(self, data):
        ...
        mollie_payment = self.acquirer_id._api_mollie_get_payment_data(acquirer_reference, force_payment=True)
    ...
    def _send_refund_request(self, amount_to_refund=None, create_refund_transaction=True):
        ...
        payment_data = self.acquirer_id._api_mollie_get_payment_data(self.acquirer_reference, force_payment=True)
        refund_data = self.acquirer_id._api_mollie_refund(amount_to_refund, self.currency_id.name, payment_data.get('id'))
        refund_tx.acquirer_reference = refund_data.get('id')
        refund_tx.fees = 0.0
    ...
    def _create_payment(self, **extra_create_values):
        ...
        if self.mollie_payment_method:
            mollie_method = self.acquirer_id.mollie_methods_ids.filtered(lambda method: method.method_code == self.mollie_payment_method)
            ...
            if mollie_method.method_code == 'voucher':
                ...
                # how payment is done.
                mollie_payment = self.acquirer_id._api_mollie_get_payment_data(self.acquirer_reference)
                ...
                if remainder_method_code:  # if there is remainder amount
                    primary_journal = mollie_method.journal_id or self.acquirer_id.journal_id
                    remainder_method = self.acquirer_id.mollie_methods_ids.filtered(lambda m: m.method_code == remainder_method_code)
    ...
    def _get_received_message(self):
        ...
        if message and self.state == 'done' and self.mollie_reminder_payment_id:
            message += _(
                ...
                self.mollie_reminder_payment_id._get_payment_chatter_link()
            )
        return message
    ...
    def _process_refund_transactions_status(self):
        ...
        for transection in refund_transactions:
            if transection.acquirer_reference:
                source_reference = transection.source_transaction_id.acquirer_reference
                if source_reference.startswith('ord_'):
                    payment_data = self.acquirer_id._api_mollie_get_payment_data(source_reference, force_payment=True)
                    source_reference = payment_data.get('id')
                refund_data = transection.acquirer_id._api_mollie_refund_data(source_reference, transection.acquirer_reference)
