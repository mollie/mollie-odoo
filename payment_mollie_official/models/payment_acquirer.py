...
class PaymentAcquirerMollie(models.Model):
    ...
    def _compute_mollie_method_fees(self, fees_by_acquirer, invoice=None, order=None, amount=None, currency=None, partner_id=None):
        ...
        :rtype: dict
        """
        mollie_acquirers = self.filtered(lambda acq: acq.provider == 'mollie')

        if not mollie_acquirers or not (order or (amount and currency and partner_id)):
        ...
        for mollie_acquirer in mollie_acquirers:
            ...
            for mollie_method in mollie_acquirer.mollie_methods_ids:
                fees_key = (mollie_acquirer, mollie_method)
                fees_by_acquirer[fees_key] = global_fees

                # this way method can override global fees (it can even removes global fees)
                if mollie_method.fees_active:
                    fees_by_acquirer[fees_key] = mollie_method._compute_fees(amount, currency, country_id)
        return fees_by_acquirer
