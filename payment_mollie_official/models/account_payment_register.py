...
class AccountPaymentRegister(models.TransientModel):
    ...
    def action_create_payments(self):
        ...
        if len(self) == 1 and self.is_mollie_refund:
            ...
            payment_data = self.mollie_transecion_id.provider_id._api_mollie_get_payment_data(self.mollie_transecion_id.provider_reference, force_payment=True)
            refund = self.mollie_transecion_id.provider_id._api_mollie_refund(self.amount, self.currency_id.name, payment_data.get('id'))
            ...
            if refund['status'] in ['pending', 'refunded'] and payments.get('res_id'):
                ...
                # TODO: automatic reconcile is not working therefore using this workaround to store refund reference on invoice.
                # payment_record.reconciled_invoice_ids.write({'mollie_refund_reference': refund['id']})
                payment_record.invoice_ids.write({'mollie_refund_reference': refund['id']})
