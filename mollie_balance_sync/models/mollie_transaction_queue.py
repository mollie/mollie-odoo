...
class MollieTransactionQueue(models.Model):
    ...
    def _cron_generate_mollie_statements_from_queue(self):
        ...
        if transaction_queue:
            self._create_bank_statements_from_queue(transaction_queue)

    def _create_bank_statements_from_queue(self, transaction_queue):
        """
    ...
    def _prepare_extra_queue_data(self):
        ...
        self.ensure_one()
        extra_data = {}
        mollie_providers = self.env['payment.provider'].search([('code', '=', 'mollie')])
        ...
        if payment_data:
            ...
            if mollie_providers:
                domain += [('provider_id', 'in', mollie_providers.ids)]
