...
patch(PosStore.prototype, {
  ...
  checkVoucher(pm, order) {
    ...
    const isMollieVoucher =
      pm.use_payment_terminal === "mollie" && pm.mollie_voucher_category;
    if (!isMollieVoucher) {
      return true;
    }
    ...
    const {voucherAmount, totalPaid} = this.getVoucherAmounts(pm, order);
    if ((totalPaid > 0 && voucherAmount === totalPaid) || order.isRefund) {
      return false;
    }
    ...
    return order
      .getOrderlines()
      .some(
        (line) => line.product_id.mollie_voucher_category === pm.mollie_voucher_category
      );
  },
});
