...
patch(PaymentScreen.prototype, {
  ...
  async sendMollieStatusCheck(line) {
    ...
    await payment_terminal.send_mollie_status_check(this.currentOrder, line.cid);
  ...
  async addNewPaymentLine(paymentMethod) {
    ...
    if (
      this.currentOrder.remainingDue >= 0 ||
      paymentMethod.use_payment_terminal != "mollie" ||
      refundOrderId == false
    ) {
      return await super.addNewPaymentLine(...arguments);
    }

    const OriginPaymentDetails = await this.env.services.orm.silent.call(
      "pos.order",
      "get_mollie_payment_details",
    ...
    if (OriginPaymentDetails.amount_remaining < -this.currentOrder.remainingDue) {
      this.env.services.dialog.add(AlertDialog, {
        ...
        body: markup(`
                    ...
                    <img style="max-width: 120px;" src="/mollie_pos_terminal/static/src/img/warning.svg" class="img img-fluid mb-2"/>
                        ...
                        <b class="text-warning"> ${this.env.utils.formatCurrency(
                          -this.currentOrder.remainingDue
                        )} </b> <b class="text-muted small mt-1"> Maximum possible amount for refund is </b>
                        <h1 class="mb-0 mt-2" style="font-size: clamp(24px, 2vw, 64px);"><b>${this.env.utils.formatCurrency(
                          OriginPaymentDetails.amount_remaining
                        )}</b></h1>
                    </div>
                `),
        size: "md",
        ...
        ConfirmationDialog,
        {
          ...
          body: markup(`
                    <div class="d-flex flex-column align-items-center">
                        ...
                        <h1 class="mb-0" style="font-size: clamp(24px, 2vw, 64px);"> <b>${this.env.utils.formatCurrency(
                          this.currentOrder.remainingDue
                        )}</b></h1>
                        <small class="text-muted"> (Total <b>${this.env.utils.formatCurrency(
                          OriginPaymentDetails.amount_remaining
                        )} </b> available for refund) </small>
  ...
  updateSelectedPaymentline(amount = false) {
    ...
    if (
      this.selectedPaymentLine.payment_method_id.mollie_voucher_category &&
      amount > this.selectedPaymentLine.payment_method_id.limit_amount
    ) {
