...
export class PaymentMollie extends PaymentInterface {
  ...
  async sendPaymentCancel(order, uuid) {
    this.env.services.dialog.add(ConfirmationDialog, {
      ...
      body: _t(
        "First cancel transaction on POS device. Only use force cancel if that fails"
      ),
  ...
  async _mollie_pay(uuid) {
    ...
    if (order.getSelectedPaymentline().amount < 0) {
      ...
      mollie_origin_transaction_id = await this.env.services.orm.silent.call(
        "pos.order",
        "get_mollie_transection_id",
  ...
  async handleMollieStatusResponse() {
    ...
    const paymentStatus = await this.env.services.orm.silent.call(
      "mollie.pos.terminal.payments",
      "get_mollie_payment_status",
      [],
      {
  ...
  _resolvePaymentStatus(state) {
    ...
    if (resolver) {
      ...
      line?.handlePaymentResponse(state);
    }
  }
