# -*- coding: utf-8 -*-

from odoo import models, api


class PosSession(models.Model):
    _inherit = 'pos.session'

    def _create_split_account_payments(self, payment_amounts_list):
        payment_aml = super()._create_split_account_payments(payment_amounts_list)
        for payment, aml in payment_aml.items():
            if payment and payment.payment_method_id.use_payment_terminal == 'mollie':
                for line in aml.move_id.line_ids:
                    line.name = line.name + ' - ' + payment.pos_order_id.pos_reference
        return payment_aml

    @api.model
    def _load_pos_data_relations(self, model, fields):
        relations = super()._load_pos_data_relations(model, fields)
        if model == 'product.template':
            relations['mollie_voucher_category'] = {
                "name": "mollie_voucher_category",
                "type": "selection",
                "compute": True,
                "related": True
            }
        return relations
