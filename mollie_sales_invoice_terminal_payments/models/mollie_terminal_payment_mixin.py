...
class MollieTerminalPaymentMixin(models.AbstractModel):
    ...
    def _compute_mollie_terminal_details(self):
        for record in self:
            ...
            if record_transactions := record.sudo().transaction_ids:
    ...
    def action_mollie_terminal_payment_status(self):
        ...
        if self.mollie_terminal_active_transaction:
            ...
            transaction_ids = self.sudo().transaction_ids.filtered(lambda t: t.provider_code == 'mollie' and t.state in ['draft', 'pending'])
            for transaction_id in transaction_ids:
                payment_data = transaction_id._send_api_request('GET', f'/payments/{transaction_id.provider_reference}')
